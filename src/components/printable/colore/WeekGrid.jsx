 ...
 */
function DayHeader({ day, date, isToday: today }) {
  ...
  return (
    <div
      ...
      `}
      style={today ? { '--tw-ring-color': colors.accent } : undefined}
    >
 ...
 */
function DayColumn({ day, date, tasks, users, isWeekend }) {
  const colors = DAY_COLORS[day.key]
    ...
    if (planning?.days) {
      ...
      tasks.forEach(task => {
        if (task.frequency === 'daily') {
          ...
          const dayIndex = Math.abs(task.id.hashCode?.() || task.id.length) % 7
