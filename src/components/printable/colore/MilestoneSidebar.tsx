...
type MilestoneStatus = 'todo' | 'in_progress' | 'done' | 'blocked'
 ...
 * Get days remaining
 */
function getDaysRemaining(dateString: string | undefined): number | null {
 ...
 */
function ProgressBar({ progress, color, size = 'md' }: ProgressBarProps): JSX.Element {
 ...
 */
function UserPastille({ userId, users }: UserPastilleProps): JSX.Element {
 ...
 */
function MilestoneCard({ milestone, users, isFocus = false }: MilestoneCardProps): JSX.Element {
  ...
  const statusConfig = STATUS_CONFIG[milestone.status || 'todo'] || STATUS_CONFIG.todo
...
export function MilestoneSidebar({
  ...
  showCompleted = false,
  maxVisible = 6,
}: MilestoneSidebarProps): JSX.Element {
  // Separate focus milestone and others
  const { focusMilestone, otherMilestones } = useMemo(() => {
    const focus = milestones.find(m => m.is_focused && m.status !== 'done')

    let others = milestones.filter(m => !m.is_focused)
                ...
                <ProgressBar
                  progress={
                    Math.round(
                      ...
                      (milestones.filter(m => m.assigned_to === 'common').length || 1)
                    )
                  }
