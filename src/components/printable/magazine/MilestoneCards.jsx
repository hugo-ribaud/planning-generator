...
export function MilestoneCards({ milestones, users, focusMilestoneId }) {
  ...
  const getIcon = (milestone) => {
    ...
    if (milestone.progress >= 75) return '&#9733;' // star
    if (milestone.progress >= 50) return '&#9650;' // triangle up
    if (milestone.progress >= 25) return '&#9679;' // circle
    return '&#9675;' // empty circle
  }

  // Get status styling
  const getStatusStyle = (status, progress) => {
    if (status === 'done') {
      return {
  ...
  const isUrgent = (milestone) => {
    if (!milestone.target_date) return false
    const days = Math.ceil((new Date(milestone.target_date) - new Date()) / (1000 * 60 * 60 * 24))
    ...
    <section className="milestone-cards">
      ...
      <div className="grid grid-cols-2 gap-4">
        {otherMilestones.map((milestone) => {
          ...
          const styles = getStatusStyle(milestone.status, milestone.progress)
