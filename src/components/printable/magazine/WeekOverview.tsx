...
export function WeekOverview({ days, users }: WeekOverviewProps): JSX.Element | null {
  if (!days || days.length === 0) return null

  // Calculate task load per day
  const getDayLoad = (day: PlanningDay): number => {
    let count = 0
  ...
  const getWeekHighlights = (): Highlight[] => {
    ...
    days.forEach(day => {
      ...
      users.forEach(user => {
        ...
        if (column?.slots) {
          column.slots.forEach(slot => {
            if (slot.task?.priority === 1 || slot.task?.priority === 'high' as unknown) {
  ...
  const getUserStats = (): Record<string, UserStats> => {
