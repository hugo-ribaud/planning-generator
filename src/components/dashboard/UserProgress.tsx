  ...
  if (userStats.length === 0) {
    return (
      <div className="bg-white rounded-xl p-6 border border-gray-200">
        ...
        <p className="text-gray-500 text-center py-4">Aucun membre configuré</p>
      </div>
    )
  }
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-3 mt-1 text-xs text-gray-500">
                <span>{user.taskCount} tâche{user.taskCount > 1 ? 's' : ''}</span>
                <span className="w-1 h-1 rounded-full bg-gray-300" />
                <span>{user.milestoneCount} objectif{user.milestoneCount > 1 ? 's' : ''}</span>
                {user.milestonesCompleted > 0 && (
                  <>
                    <span className="w-1 h-1 rounded-full bg-gray-300" />
                    <span className="text-green-600">{user.milestonesCompleted} terminé{user.milestonesCompleted > 1 ? 's' : ''}</span>
                  </>
                )}
              </div>
            </div>
